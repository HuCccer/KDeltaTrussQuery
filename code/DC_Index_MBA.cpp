...
vector<vector<vector<TreeNode*>>> preNode;
...
void dc_Index_Construction(string dataset_path){
	...
	preNode.resize(k_max+1,vector<vector<TreeNode*>>(t_max+1));
	for(uint32_t t = t_max ; t > 0 ; --t){
		for(auto tri:time2triangle[t]){
			...
			//update ts
			if(k1<=k2 && k1<=k3) {
				if(ts_[e1]-- == k1) q.push(e1);
			} 
			if(k2<=k1 && k2<=k3) {
				if(ts_[e2]-- == k2) q.push(e2);
			} 
			if(k3<=k1 && k3<=k2) {
				if(ts_[e3]-- == k3) q.push(e3);
			}
			...
			while(!q.empty()){
				...
				while (p1 < adj_[v1].size() && p2 < adj_[v2].size()) {
					...
					if (adj_[v1][p1].vid == adj_[v2][p2].vid) {
						...
						if(Mc[mineId].find(maxeId) != Mc[mineId].end()) {
							++p1; ++p2;
							continue;
						}
						nfound++;
						...
						tse++;
						if(k_[e1] >= k && k_[e2] >= k) {
							if(k_[e1] == k && Ins[e1] == false && ts_[e1]-- == k_[e1]) {
								q.push(e1); Ins[e1] = true;
							}
							if(k_[e2] == k && Ins[e2] == false && ts_[e2]-- == k_[e2]) {
			...
			if(k == k_max){
				if(D[k].size() != 0){
					...
					for(uint32_t i = k - 1; i >= 1; --i){
						...
						if(isSave[i] == false){
							DC_index[i].emplace_back(make_pair(t,n));
							isSave[i] = true;
						}
					} 
					for(auto node: preNode[k][t]){
						node->next = n;
					}
					DC_index[k].emplace_back(make_pair(t,n));	
					preNode[k][t-1].emplace_back(n);
				}
				else{
					for(auto node: preNode[k][t]){
						preNode[k][t-1].emplace_back(node);
					}
				}
			}
			else{
				...
				if(x != 0){
					...
					if(left == 0){
						...
						for(auto node: preNode[k][t]){
							preNode[k+1][t].emplace_back(node);
						}
					}
...
int main(int args, char* argv[]) {
	...
	// 	cin>>k>>t;
	// 	if(k > k_max){
	// 		cout<<"the input is illegal,k is less than:"<<k_max<<" please input again!"<<endl;
	// 		continue;
	// 	}
	// 	time_t t1 = clock();
	// 	queryKDeltaTruss(k,t);
	// 	time_t t2 = clock();
	// 	cout << "-----------------------------" << endl; 
